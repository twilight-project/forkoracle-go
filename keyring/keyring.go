...
type Signer interface {
	...
	SignTx(accountName string, amounts []uint64, tx *wire.MsgTx, script []byte) []string
}
...
func (ks keystore) SignTx(accountName string, amounts []uint64, tx *wire.MsgTx, script []byte) []string {
	signatures := []string{}
	key, err := ks.Key(accountName)
	var priv types.PrivKey
	switch i := key.(type) {
	case localInfo:
		if i.PrivKeyArmor == "" {
			return []string{"private key not available"}
		}
	...
	privkey, _ := btcec.PrivKeyFromBytes(btcec.S256(), priv.Bytes())
